...
- [Template literal](https://www.typescriptlang.org/docs/handbook/2/template-literal-types.html)
- [Recursive](https://www.executeprogram.com/courses/advanced-typescript/lessons/recursive-types)
...
type OptionalFlags<Type> = {
	[Propertys in keyof Type]: boolean;
}
...
type NewOptionalType = OptionalType<Features>
...
type Example2 = Dog extends Machine ? true : false
```

#### **Нотатки:**
> *Mapped Types*:
> 	**Mapping Modifiers** — за допомогою + та - перед опціональним модифікатором, його можна або встановити, або прибрати, якщо перед опціональним типом нічого немає, це автоматично +
> 	**Key remaping** — можна перейменувати ключи прямо в маппінгу за допомогою ==as== 
...
// Conditional type
type ConditionalType<Type extends SomeType ? true : false>

// Literal type
type LiteralType = "left" | "right"
```

#### **Нотатки:**
> Mapped type — перебирає всі типи існуючого інтерфейса, і трансормує його типи
> Conditional type — перевіряє чи один тип наслідує інший, тобто тернарний тип в TS
> Literal type — це тип який може бути тільки чимось конкретним

---

### Next JS
#### **Нотатки:**
> Route Group — це папка яка слугує, як папка групи, яка не впливає на маршрут файлів в URL
...
*Шаблон створено: 2026-05-15 | Оновлено: 2026-05-15
