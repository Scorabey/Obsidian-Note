...
FROM orders
LEFT JOIN users ON isers.id = orders.user_id
...
SELECT users.name, orders.date
FROM orders
RIGHT JOIN users ON isers.id = orders.user_id
...
r.name AS referal
FROM referals u
LEFT JOIN referals r ON r.referal_id = u.id;
```

#### **Нотатки:**
> **INNER JOIN** — виводить тільки ті рядки, для яких знайшлася відповідна зв'язана інформація в обох таблицях. Якщо зв'язаної записи немає, такий рядок не виводиться.

> **LEFT JOIN** — виводить **усі рядки з лівої таблиці** та відповідні дані з правої. Якщо для якогось рядка немає відповідного запису в правій таблиці, то поля з правої таблиці матимуть значення `NULL`.

> **RIGHT JOIN** — виводить **усі рядки з правої таблиці** та відповідні дані з лівої. Якщо для якогось рядка немає відповідного запису в лівій таблиці, то поля з лівої таблиці матимуть значення `NULL`.

> Self Join — логічно об'єднує дані однох таблиці в дві окремі, наприклад щоб построїти логічний зв'язок між користувачем, та його рефералами
...
*Шаблон створено: 2026-06-12 | Оновлено: 2026-06-12
